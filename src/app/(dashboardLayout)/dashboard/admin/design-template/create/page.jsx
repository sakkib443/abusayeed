...
const INDUSTRY_OPTIONS = [
  'eCommerce', 'Restaurant', 'Portfolio', 'Corporate', 'Agency',
  'Healthcare', 'Education', 'Real Estate', 'Technology', 'Fashion',
  'Travel', 'Finance', 'Entertainment', 'Sports', 'Non-Profit',
  'Wedding', 'Photography', 'Blog', 'Magazine', 'Other'
];
...
export default function CreateDesignTemplatePage() {
        ...
        try {
          ...
          if (data.data) {
            ...
            reset({
              ...
              // New optional fields
              tags: sw.tags?.length ? sw.tags : [''],
              colors: sw.colors?.length ? sw.colors : [''],
              industry: sw.industry || '',
              fileSize: sw.fileSize || '',
              dimensions: sw.dimensions || '',
              isEditable: sw.isEditable ?? true,
              softwareVersion: sw.softwareVersion || '',
              layered: sw.layered || false,
              responsive: sw.responsive || false,
              fontIncluded: sw.fontIncluded || false,
            });
          }
        } catch (err) {
  ...
  const onSubmit = async (values) => {
    ...
    const payload = {
      ...
      colors: cleanArray(values.colors),
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className={cardClass}>
              <div className="flex justify-between items-center mb-4">
                <h2 className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-800'} flex items-center gap-2`}>
                  <FiTag size={16} className="text-blue-500" /> Tags (SEO)
                </h2>
                <button type="button" onClick={() => tagFields.append('')} className="p-1.5 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition-colors">
                  <FiPlus size={14} />
                </button>
              </div>
              <div className="space-y-2">
                {tagFields.fields.map((field, idx) => (
                  <div key={field.id} className="flex gap-2">
                    <input {...register(`tags.${idx}`)} className={inputStyle('tags')} placeholder="modern, minimal, dark..." />
                    <button type="button" onClick={() => tagFields.remove(idx)} className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-md transition-colors">
                      <FiTrash2 size={16} />
                    </button>
                  </div>
                ))}
            ...
            <div className={cardClass}>
              <div className="flex justify-between items-center mb-4">
                <h2 className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-800'} flex items-center gap-2`}>
                  <FiLayers size={16} className="text-pink-500" /> Colors
                </h2>
                <button type="button" onClick={() => colorFields.append('')} className="p-1.5 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition-colors">
                  <FiPlus size={14} />
                </button>
              </div>
              <div className="space-y-2">
                {colorFields.fields.map((field, idx) => (
                  <div key={field.id} className="flex gap-2">
                    <input {...register(`colors.${idx}`)} className={inputStyle('colors')} placeholder="#FF5733, blue, etc." />
                    <button type="button" onClick={() => colorFields.remove(idx)} className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-md transition-colors">
                      <FiTrash2 size={16} />
                    </button>
                  </div>
                ))}
          ...
          <div className={cardClass}>
            <h2 className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-800'} mb-4 flex items-center gap-2`}>
              <FiSettings size={16} className="text-purple-500" /> Technical Details
            </h2>
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className={labelStyle('industry')}>Industry</label>
                  <select {...register('industry')} className={inputStyle('industry')}>
                    <option value="">Select Industry</option>
                    {INDUSTRY_OPTIONS.map(i => <option key={i} value={i}>{i}</option>)}
                  </select>
                </div>
                <div>
                  <label className={labelStyle('fileSize')}>File Size</label>
                  <input {...register('fileSize')} className={inputStyle('fileSize')} placeholder="25MB, 1.2GB..." />
                </div>
                <div>
                  <label className={labelStyle('dimensions')}>Dimensions</label>
                  <input {...register('dimensions')} className={inputStyle('dimensions')} placeholder="1920x1080, A4..." />
                </div>
              </div>

              <div>
                <label className={labelStyle('softwareVersion')}>Required Software Version</label>
                <input {...register('softwareVersion')} className={inputStyle('softwareVersion')} placeholder="Photoshop CC 2023, Figma 2024..." />
              </div>

              {/* Checkboxes */}
              <div className={`border-t ${isDark ? 'border-slate-700' : 'border-gray-200'} pt-4`}>
                <p className={`text-xs font-medium ${isDark ? 'text-slate-400' : 'text-gray-600'} mb-3`}>File Properties</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {/* Is Editable */}
                  <label className={`flex items-center gap-2 p-3 rounded-md cursor-pointer ${isDark ? 'bg-slate-700/50 hover:bg-slate-700' : 'bg-gray-50 hover:bg-gray-100'} transition-colors`}>
                    <input type="checkbox" {...register('isEditable')} className="hidden" />
                    <div className={`w-4 h-4 border-2 rounded flex items-center justify-center transition-colors ${watch('isEditable') ? 'bg-blue-500 border-blue-500' : isDark ? 'border-slate-600' : 'border-gray-300'}`}>
                      {watch('isEditable') && <FiCheck className="text-white" size={10} />}
                    </div>
                    <span className={`text-xs ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Editable</span>
                  </label>

                  {/* Layered */}
                  <label className={`flex items-center gap-2 p-3 rounded-md cursor-pointer ${isDark ? 'bg-slate-700/50 hover:bg-slate-700' : 'bg-gray-50 hover:bg-gray-100'} transition-colors`}>
                    <input type="checkbox" {...register('layered')} className="hidden" />
                    <div className={`w-4 h-4 border-2 rounded flex items-center justify-center transition-colors ${watch('layered') ? 'bg-blue-500 border-blue-500' : isDark ? 'border-slate-600' : 'border-gray-300'}`}>
                      {watch('layered') && <FiCheck className="text-white" size={10} />}
                    </div>
                    <span className={`text-xs ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Layered</span>
                  </label>

                  {/* Responsive */}
                  <label className={`flex items-center gap-2 p-3 rounded-md cursor-pointer ${isDark ? 'bg-slate-700/50 hover:bg-slate-700' : 'bg-gray-50 hover:bg-gray-100'} transition-colors`}>
                    <input type="checkbox" {...register('responsive')} className="hidden" />
                    <div className={`w-4 h-4 border-2 rounded flex items-center justify-center transition-colors ${watch('responsive') ? 'bg-blue-500 border-blue-500' : isDark ? 'border-slate-600' : 'border-gray-300'}`}>
                      {watch('responsive') && <FiCheck className="text-white" size={10} />}
                    </div>
                    <span className={`text-xs ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Responsive</span>
                  </label>

                  {/* Font Included */}
                  <label className={`flex items-center gap-2 p-3 rounded-md cursor-pointer ${isDark ? 'bg-slate-700/50 hover:bg-slate-700' : 'bg-gray-50 hover:bg-gray-100'} transition-colors`}>
                    <input type="checkbox" {...register('fontIncluded')} className="hidden" />
                    <div className={`w-4 h-4 border-2 rounded flex items-center justify-center transition-colors ${watch('fontIncluded') ? 'bg-blue-500 border-blue-500' : isDark ? 'border-slate-600' : 'border-gray-300'}`}>
                      {watch('fontIncluded') && <FiCheck className="text-white" size={10} />}
                    </div>
                    <span className={`text-xs ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Font Included</span>
                  </label>
                </div>
              </div>
            </div>
