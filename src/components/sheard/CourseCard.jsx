...
const CourseCard = ({ course, view = "grid" }) => {
      ...
      >
        ...
        <div className="relative w-full md:w-[35%] h-56 md:h-auto shrink-0 overflow-hidden p-3">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/20 backdrop-blur-[1px]">
            <Link href={`/courses/${courseId}`} className="w-14 h-14 bg-white/30 backdrop-blur-md rounded-full flex items-center justify-center text-white border border-white/50 hover:bg-[#003ECB] hover:border-[#003ECB] transition-all hover:scale-110">
        ...
        <div className="flex-1 p-6 border-r border-slate-50 dark:border-white/5 flex flex-col justify-center">
          <Link href={`/courses/${courseId}`}>
            <h3 className={`text-xl font-bold text-slate-800 dark:text-white leading-tight mb-2 hover:text-[#003ECB] transition-colors ${bengaliClass}`}>
              {title}
            </h3>
          </Link>
        ...
        <div className="w-full md:w-[25%] p-6 bg-slate-50/50 dark:bg-white/5 flex flex-col items-center justify-center text-center gap-1 border-l border-slate-100 dark:border-white/5">
          <div className="flex w-full justify-end gap-2 mb-2 text-slate-400">
            <button className="hover:text-[#003ECB] transition-colors"><LuList size={18} /></button>
            <button className="hover:text-amber-500 transition-colors"><LuHeart size={18} /></button>
          </div>

          <div className="text-3xl font-bold text-[#003ECB] font-serif italic mb-1">
          ...
          <div className="flex items-center gap-3 w-full">
            <button
              ...
              disabled={isAdded}
              className={`p-2.5 border rounded-xl transition-all shadow-sm ${isAdded ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white dark:bg-white/10 border-slate-200 dark:border-white/10 text-slate-600 dark:text-white hover:text-[#003ECB] hover:border-[#003ECB]'}`}
            >
              {isAdded ? <LuCheck size={20} /> : <LuShoppingCart size={20} />}
            </button>
            <Link
              href={`/courses/${courseId}`}
              className="flex-1 py-2.5 bg-white dark:bg-white/10 border border-[#003ECB] text-[#003ECB] rounded-xl text-sm font-medium hover:bg-[#003ECB] hover:text-white transition-all shadow-sm flex items-center justify-center gap-2"
            >
              Details
            </Link>
          </div>
  ...
  return (
    ...
    >
      <div className={`relative h-full bg-white dark:bg-[#0d0d0d] rounded-3xl border border-slate-100 dark:border-white/10 overflow-hidden hover:shadow-2xl hover:border-[#003ECB]/20 transition-all duration-500 flex flex-col shadow-lg shadow-black/5`}>
        ...
        <div className="relative h-48 w-full overflow-hidden shrink-0">
          ...
          <div className="absolute top-4 left-4">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold text-white shadow-lg backdrop-blur-md bg-[#003ECB]/80`}>
              <LuSparkles size={10} />
              {type.toUpperCase()}
            </span>
          </div>
          ...
          >
            <Link href={`/courses/${courseId}`} className="w-14 h-14 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white border-2 border-white/40 hover:bg-[#003ECB] hover:border-[#003ECB] transition-all hover:scale-110 shadow-2xl">
              <LuPlay className="ml-1" size={20} fill="currentColor" />
            </Link>
          </motion.div>
        </div>
        ...
        <div className="p-6 flex flex-col flex-1 relative">
          ...
          <Link href={`/courses/${courseId}`} className="mb-4 block">
            <h3 className={`md:text-lg font-bold text-slate-800 dark:text-white leading-snug line-clamp-2 hover:text-[#003ECB] transition-colors ${bengaliClass}`}>
              {title}
            </h3>
          </Link>
          ...
          <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-50 dark:border-white/5">
            <div className="flex items-center gap-3 text-[10px] font-medium text-slate-500">
              <div className="flex items-center gap-1">
                <LuBookOpenCheck className="text-[#003ECB]" size={12} />
                <span>{lessons}</span>
              </div>
              <div className="flex items-center gap-1">
                <LuUsers className="text-[#003ECB]" size={12} />
                <span>{students}</span>
              </div>
            </div>
            <div className="flex items-center gap-1">
          ...
          <div className="flex items-center justify-between mt-auto gap-4">
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xl font-serif italic font-bold text-[#003ECB]">
                  ৳{(discountPrice || price).toLocaleString()}
                </span>
                {discountPrice && (
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isAdded
                  ...
                  : 'bg-slate-50 dark:bg-white/5 text-slate-600 dark:text-white hover:bg-[#003ECB] hover:text-white'
                  }`}
              >
              ...
              <Link
                href={`/courses/${courseId}`}
                className="w-10 h-10 rounded-full bg-[#003ECB] text-white flex items-center justify-center hover:scale-110 transition-transform shadow-lg shadow-[#003ECB]/20"
              >
